...
from utils.input_welder import read_welders
from utils.date_utils import date_difference

if __name__ == '__main__':
    today_date = read_today_date()
    welders = read_welders()
    expired_authorizations = {}
    for welder in welders:
        first_name, last_name, company, expiration_date = welder
        ...
        if date_difference(today_date=today_date, expiration_date=expiration_date):
            if company in expired_authorizations:
                ...
                expired_authorizations[company].append((first_name, last_name, expiration_date))
            else:
                # if there is no company as key, add it
                expired_authorizations[company] = [(first_name, last_name, expiration_date)]
    print()
    for company, welders in expired_authorizations.items():
        ...
        for first_name, last_name, expiration_date in welders:
            day, month, year = expiration_date
            print(f"      {first_name} {last_name}'s authorization expires on {day}/{month}/{year}")
        print()